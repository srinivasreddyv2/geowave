...
public class MultiAdapterServerMergeStrategy<T extends Persistable> implements ServerMergeStrategy, Mergeable {
...
// ID pairs is to allow for multiple adapters to share the same sample model
	...
	public MultiAdapterServerMergeStrategy(SingleAdapterServerMergeStrategy singleAdapterMergeStrategy) {
	...
	public void merge(final Mergeable merge) {
		synchronized (this) {
			if ((merge != null) && (merge instanceof MultiAdapterServerMergeStrategy)) {
				...
				mergeMaps(sampleModels, adapterIdToSampleModelKey, other.sampleModels, other.adapterIdToSampleModelKey);
				mergeMaps(childMergeStrategies, adapterIdToChildMergeStrategyKey, other.childMergeStrategies,
...
// exist in 'this'
	...
	@SuppressFBWarnings(value = { "DLS_DEAD_LOCAL_STORE" }, justification = "Incorrect warning, sampleModelBinary used")
	@Override
	...
	public void fromBinary(final byte[] bytes) {
		...
		for (int i = 0; i < sampleModelAdapterIdSize; i++) {
			adapterIdToSampleModelKey.put(buf.getShort(), buf.getInt());
		}
		...
		for (int i = 0; i < mergeStrategyAdapterIdSize; i++) {
			adapterIdToChildMergeStrategyKey.put(buf.getShort(), buf.getInt());
		}
	}
...
//}
	...
	@Override
	public void merge(final RasterTile thisTile, final RasterTile nextTile, final short internalAdapterId) {
